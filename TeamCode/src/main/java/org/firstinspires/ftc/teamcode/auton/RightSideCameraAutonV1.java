...
@Autonomous
@Disabled
public class RightSideCameraAutonV1 extends LinearOpMode {
    ...
    private void initPID() {
        ...
        this.turretPid = new Pid(new Pid.Coefficients(0.005, 0.02, 0.0, 1.0, 0.05),
                () -> {
                    if (this.turretRunMode == TurretRunMode.RUN_TO_POSITION) return 0.0;
                    return this.cameraComponent.getTarget().x - C.CAMERA_WIDTH / 2.0;
                },
                factor -> {
                    if (this.turretRunMode == TurretRunMode.RUN_TO_POSITION) return;
                    this.turretComponent.setPower(M.clamp(factor, -0.7, 0.7));
                });
        ...
        this.turretPid = new Pid(new Pid.Coefficients(3.2, 1.2, 0.0),
                () -> {
                    if (this.turretRunMode == TurretRunMode.ALIGN_POLE) return 0.0;
                    return this.targetTurretPosition - this.turret.getCurrentPosition();
                },
                factor -> {
                    if (this.turretRunMode == TurretRunMode.ALIGN_POLE) return;
...
//        telemetry.addData("limit", this.turretSensor.isPressed());
    ...
    public void runOpMode() throws InterruptedException {
        ...
        if (opModeIsActive()) {
            ...
            for(int i = 0; i < 80; i++) this.updateMotor();
            //end
            score();
            updateAll();
            preIntakeMode();
