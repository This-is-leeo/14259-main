...
@Autonomous
//@Disabled
public class RightSideCameraAuton extends LinearOpMode {
    ...
    private AsyncThreaded turretThread;
    ...
    private Pid turretPid1;
    private Pid turretPid2;
    ...
    // Modifications 3/2/23 - please re-organize
    private CameraComponent cameraComponent;
    private static enum TurretRunMode { RUN_TO_POSITION, ALIGN_POLE };
    private TurretRunMode turretRunMode = TurretRunMode.RUN_TO_POSITION;
    ...
    private void initCameraComponent() {
        this.cameraComponent = new CameraComponent(null, hardwareMap);
    }
    private void initPID() {
        ...
        this.turretPid1 = new Pid(new Pid.Coefficients(0.002, 0.0, 0.0),
                () -> {
                    if (this.turretRunMode == RightSideCameraAuton.TurretRunMode.RUN_TO_POSITION) return 0.0;
                    return this.cameraComponent.getTarget().x - C.CAMERA_WIDTH / 2.0;
                },
                factor -> {
                    if (this.turretRunMode == RightSideCameraAuton.TurretRunMode.RUN_TO_POSITION) return;
        ...
        this.turretPid2 = new Pid(new Pid.Coefficients(3.2, 1.2, 0.0),
                () -> {
                    if (this.turretRunMode == RightSideCameraAuton.TurretRunMode.ALIGN_POLE) return 0.0;
                    return this.targetTurretPosition - this.turret.getCurrentPosition();
                },
                factor -> {
                    if (this.turretRunMode == RightSideCameraAuton.TurretRunMode.ALIGN_POLE) return;
...
//        telemetry.addData("limit", this.turretSensor.isPressed());
        telemetry.addData("turret Mode", this.turretRunMode );
        telemetry.update();
    }
    private void updateTurret(){
        if (this.turretRunMode == TurretRunMode.RUN_TO_POSITION || this.turretRunMode == TurretRunMode.ALIGN_POLE) {
            this.turretPid1.update();
            this.turretPid2.update();
        }
        this.turret.update();
    }
    ...
    private void initAsync() {
        ...
        turretThread = new AsyncThreaded(() -> {})
                .then(() -> {
                    while (!AsyncThreaded.stopped) {
                        this.updateTurret();
                    }
    ...
    public void runOpMode() throws InterruptedException {
        ...
        this.initAll();
        FtcDashboard.getInstance().startCameraStream(this.cameraComponent.getWebcam(), 0);
        resetDepositPosition();
        while (!isStarted() && !isStopRequested()) {
           ArrayList<AprilTagDetection> currentDetections = cameraComponent.getDetections();
...
//            this.g.updateAll();
        ...
        turretRunMode = TurretRunMode.RUN_TO_POSITION;
        if (opModeIsActive()) {
            AsyncThreaded.stopped = false;
            this.initAsync();
            this.turretThread.run();
            //this.Randomization.set(randomization);
            this.drivetrainThread.run();
            this.linSlideThread.run();
            targetTurretPosition = 0.63;
            targetPitchPosition = 0.54;
            for(int i = 0; i < 50; i++) {
                this.updateMotor();
//                this.updateTurret();  b
            }
//            this.aligning = true;
            while(!C.parked.get()) this.updateMotor();
            //start
            this.turretRunMode = TurretRunMode.ALIGN_POLE;
            while (this.turret.isBusy());
//            while (this.turret.isBusy());
            //end
            score();
            updateAll();
            ...
            intakeBack();
            this.updateAll();
            this.turretRunMode = TurretRunMode.RUN_TO_POSITION;
            ...
            this.pitchRTP = true;
            C.parked.set(false);
            AsyncThreaded.stopped = true; // I propose moving this to the end! -Ethan
            if(randomization == 1){
            ...
            while(this.drive.isBusy() || this.turret.isBusy()){
                this.updateAll();
                this.updateDrive();
                this.updateTurret();
            }
            if(randomization == 1) this.drive.addTargetR(Math.toRadians(-90));
            while(this.drive.isBusy() || this.turret.isBusy()){
                this.updateAll();
                this.updateDrive();
                this.updateTurret();
